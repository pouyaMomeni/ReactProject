...
import QuestionMbti from './QuestionMbti';
...
const MbtiTest = (props) => {
    ...
    useEffect(() => {
        getTestQuestions("mbti").then(ques => {
            getTestState(identifier, "mbti").then(res => {
                let lastAnswered;
    ...
    const onResultClick = () => {
        Promise.all([getTestState(identifier, "mbti")]).then(([neoState]) => {
            if (!neoState.done)
                return toast.warn("باید آزمون مقیاس شخصیتی را کامل کنید");
            else props.history.push("/resultMbti");
        }).catch(err => {
            console.log(err)
        });
    };
        ...
        <Box sx={{display:'flex',flexDirection:'column',alignItems:'center',gridGap:'2rem',padding:'1rem',bgcolor:'white ',width:'100%',marginBottom:'8rem'}}>
            <Box className='border-question' sx={{display:'flex',alignItems:'center',width:{xs:'100%',sm:'100%',md:'100%',lg:'50%'},justifyContent:'space-around'}}>
                <Box sx={{display:'flex',alignItems:'center',gridGap:'10px'}}>
                    <Box sx={{fontSize:'0.8rem',bgcolor:'#006666',alignItems:'center',display:'flex',padding:'0.3rem',color:'white'}}>آزمون Mbti</Box>
            ...
            <QuestionMbti isNeo={true} onResultClick={onResultClick} classes={classes} identifier={identifier}
