...
import Question16Fp from './Question16Fp';
...
const Fp16 = (props) => {
    ...
    useEffect(() => {
        getTestQuestions("16fp").then(ques => {
            getTestState(identifier, "16fp").then(res => {
    ...
    const onResultClick = () => {
        Promise.all([getTestState(identifier, "16fp")]).then(([neoState]) => {
            if (!neoState.done)
                return toast.warn("باید آزمون کتل را کامل کنید");
            else props.history.push("/resultFp16");
        }).catch(err => {
            console.log(err)
        });
    };
        ...
        <Box sx={{display:'flex',flexDirection:'column',alignItems:'center',gridGap:'2rem',padding:'1rem',bgcolor:'white ',width:'100%',marginBottom:'8rem'}}>
            <Box className='border-question' sx={{display:'flex',alignItems:'center',width:{xs:'100%',sm:'100%',md:'100%',lg:'50%'},justifyContent:'space-around'}}>
                <Box sx={{display:'flex',alignItems:'center',gridGap:'10px'}}>
                    <Box sx={{fontSize:'0.8rem',bgcolor:'#006666',alignItems:'center',display:'flex',padding:'0.3rem',color:'white'}}>آزمون 16FP</Box>
            ...
            <Question16Fp isNeo={true} onResultClick={onResultClick} classes={classes} identifier={identifier}
