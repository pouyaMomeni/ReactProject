    ...
    >
      ...
      <ul className={classes.options}>
        ...
        {q.options && (
          <>
            <li
              onClick={() => !loading && selectAns(q.options[4].id,q.options[4].option_place)}
              className={q.ans === 5 && classes.optionActive}
            >
              {q.options[4].option_text}
            </li>
            <li
              onClick={() => !loading && selectAns(q.options[3].id,q.options[3].option_place)}
              className={q.ans === 4 && classes.optionActive}
            >
      ...
      {/*<RadioGroup aria-label="quiz" name="quiz" style={{marginBottom: '1rem'}}>*/}
      {/*    {q.options.map(item => <FormControlLabel*/}
      {/*        control={<Radio onChange={e => onChange(e, item.id)} checked={q.ans === item.id}/>}*/}
      {/*        label={(item.id % 5 ? (item.id % 5) : 5) + ". " + item.option_text}/>)}*/}
      {/*</RadioGroup>*/}
      ...
      {index === questionsLength - 1 && q.ans && (
        <Grid container justify={"center"}>
          ...
          {/* {isNeo ?
                    sessionStorage.getItem("single")==="false" &&
                    <Button size={"small"} className={classes.lastQ} variant={"contained"} onClick={enrich}>آزمون
                        Enrich</Button> :
                    <Button size={"small"} className={classes.lastQ} variant={"contained"} onClick={neo}>آزمون
                        Neo</Button>
      ...
      {/*{index === questionsLength - 1 &&*/}
      {/*<Button style={{marginRight: '1rem'}} color={"secondary"} variant={"contained"}>اتمام پرسش نامه</Button>}*/}
    </Box>
  );
};
