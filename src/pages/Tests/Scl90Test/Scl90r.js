...
import QuestionL90 from './QuestionScl90';
...
const Scl90r = (props) => {
    ...
    useEffect(() => {
        getTestQuestions("scl-90-r").then(ques => {
            getTestState(identifier, "scl-90-r").then(res => {
                let lastAnswered;
    ...
    const onResultClick = () => {
        Promise.all([getTestState(identifier, "scl-90-r")]).then(([neoState]) => {
            props.history.push("/resultScl90");
        }).catch(err => {
            console.log(err)
        });
    };
    ...
    return (
        <>
            ...
            <Box sx={{display:'flex',flexDirection:'column',alignItems:'center',gridGap:'2rem',padding:'1rem',bgcolor:'white ',width:'100%',marginBottom:'8rem'}}>
                <Box className='border-question' sx={{display:'flex',alignItems:'center',width:{xs:'100%',sm:'100%',md:'100%',lg:'50%'},justifyContent:'space-around'}}>
                    <Box sx={{display:'flex',alignItems:'center',gridGap:'10px'}}>
                        <Box sx={{fontSize:'0.8rem',bgcolor:'#006666',alignItems:'center',display:'flex',padding:'0.3rem',color:'white'}}>آزمون Scl-90</Box>
            ...
            <QuestionL90 isNeo={true} onResultClick={onResultClick} classes={classes} identifier={identifier}
