...
const EnrichTest = (props) => {
    ...
    const onResultClick = () => {
        Promise.all([getTestState(identifier, "Enrich")]).then(([neoState]) => {
            if (!neoState.done)
                return toast.warn("باید آزمون انریچ را کامل کنید");
            else props.history.push("/resultEenrichSingle");
        }).catch(err => {
            console.log(err)
        });
    };
        ...
        <Box sx={{display:'flex',flexDirection:'column',alignItems:'center',gridGap:'2rem',padding:'1rem',bgcolor:'white ',width:'100%',marginBottom:'8rem'}}>
            <Box className='border-question' sx={{display:'flex',alignItems:'center',width:{xs:'100%',sm:'100%',md:'100%',lg:'50%'},justifyContent:'space-around'}}>
                <Box sx={{display:'flex',alignItems:'center',gridGap:'10px'}}>
                    <Box sx={{fontSize:'0.8rem',bgcolor:'#006666',alignItems:'center',display:'flex',padding:'0.3rem',color:'white'}}>آزمون Enrich</Box>
